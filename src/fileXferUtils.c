...
void write_uint32_le(uint32_t num, uint8_t *ptr) {
    ptr[3] = (uint8_t)((num >> 24) & 0xFF);
    ptr[2] = (uint8_t)((num >> 16) & 0xFF);
    ptr[1] = (uint8_t)((num >> 8) & 0xFF);
    ptr[0] = (uint8_t)(num & 0xFF);
}
...
int crc32_compute_file(FILE *file, uint32_t *out_crc32) {
    ...
    *out_crc32 = 0;
    while (1) {
        ...
        *out_crc32 = crc32_compute_buf(*out_crc32, buf, buf_len);
    }
    return 0;
}
...
void print_str_hex(uint8_t* str, uint32_t str_len, int (*print_fp)(const char *fmt, ...)) {
    uint8_t print_buf[str_len*2 + 2];
    memset(print_buf, 0, str_len*2 + 2);
    uint32_t buf_ind = 0;
    for(uint32_t i = 0; i < str_len; i++){
        sprintf((char*)&print_buf[buf_ind], "%02X", str[i]);
        buf_ind = buf_ind + 2;
    }
    print_buf[str_len*2] = '\n';
    print_fp("%s", print_buf);
}
